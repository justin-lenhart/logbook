"""Model -> Grist cell-value mapping.

Mirrors ``airtable_mapper.py`` with Grist's data formats:
- Date columns take epoch seconds at UTC midnight (int).
- DateTime columns take epoch seconds (int).
- ChoiceList cells are encoded ``["L", value, ...]``.
- Reference columns take Grist row ids (set by the sync layer, not here).
Only STORED columns are mapped — formula columns compute server-side.
"""
...
def format_grist_date(value: date) -> int:
    """Grist Date cell: epoch seconds at UTC midnight."""
    return int(datetime(value.year, value.month, value.day, tzinfo=timezone.utc).timestamp())


def format_grist_datetime(value: datetime) -> int:
    """Grist DateTime cell: epoch seconds. Naive datetimes are rejected upstream
    for flights; for other callers we follow Python semantics (assume local)."""
    return int(value.timestamp())


def encode_choice_list(values: list[str]) -> list[Any]:
    return ["L", *values]
...
def map_duty_period_fields(
    ...
    *,
    ...
    fields: dict[str, Any] = {
        ...
        F.F_DUTY_DATE: format_grist_date(duty.duty_date),
...
def map_flight_fields(flight: PlannedFlightRecord) -> dict[str, Any]:
    ...
    fields: dict[str, Any] = {
        ...
        F.F_FLIGHT_DATE: format_grist_date(flight.duty_date),
        F.F_FLIGHT_NUMBER: flight.flight_number,
        F.F_FLIGHT_OUT_TIME: format_grist_datetime(flight.out_time),
        F.F_FLIGHT_IN_TIME: format_grist_datetime(flight.in_time),
...
def map_import_batch_fields(
    ...
    *,
    ...
    return {
        ...
        F.F_BATCH_IMPORT_DATETIME: format_grist_datetime(imported_at),
